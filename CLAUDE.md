...
### מה זה
קוויז onboarding שמופיע לאחר הרכישה הראשונה — לפני הכניסה לדשבורד. 4 שאלות, מסך אחד לכל שאלה, עם תמונות של נטלי. בסוף: "המסלול שלך" — 4 קורסים מותאמים אישית.

### השאלות (מאושרות)
1. **רמה** — מתחילה / בינונית / מקצועית / (אופציה 4)
2. **אתגר** — עיניים / עור / שפתיים / כלל הפנים
3. **תחום עניין** — כלות / ערב / צילום / יומי
4. **יעד** — עסק רווחי / רמה גבוהה / למידה מסודרת / ביטחון עצמי

### עיצוב
- מסך מלא, רקע כהה כמו שאר הפלטפורמה
- כרטיסים 2x2: תמונת נטלי + תווית קצרה על כל תשובה
- פס התקדמות עדין בראש (שלב 1/4)
- Fade transition בין שאלות
- מסך סיום: "המסלול שלך, [שם]" — 4 קורסים עם הסבר קצר למה כל אחד

### תמונות נדרשות (מחכות לשי)
16 תמונות מנטלי — 4 שאלות × 4 תשובות:
- שאלה 1 (רמה): לוק פשוט / יומי מוגמר / דרמטי / ...
- שאלה 2 (אתגר): close-up עין / עור / שפתיים / פנים שלמות
- שאלה 3 (תחום): כלה / ערב / סט צילום / חוץ יומי
- שאלה 4 (יעד): נטלי עם לקוחה / תוצאה מרשימה / שיעור / חיוך גאה

### מה צריך לבנות (לא התחלנו)
1. טבלה `quiz_answers` ב-Supabase (user_id + answers JSON)
2. שדות tags בקורסים: `level`, `topics`, `domains` (מערך) — גם ב-DB וגם באדמין
3. 4 מסכי שאלות + מסך תוצאה
4. לוגיקת matching: ניקוד לכל קורס לפי תגיות תואמות
5. ניתוב: אחרי רכישה ראשונה → /onboarding → דשבורד עם מסלול

### מה שי צריך לעשות לפני הפיתוח
- [ ] לספק 16 תמונות מנטלי (ראה רשימה למעלה)
- [ ] למלא טבלת tags לכל קורס: רמה / נושא / תחום
